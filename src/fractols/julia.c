...
/*   Updated: 2016/12/20 17:35:14 by mploux           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_pixel(t_data *data, t_vec2 z, t_vec2 pix, int iteration)
{
	double	i;
	double	old_zx;
	t_vec2	c;
	...
	i = 0;
	c.x = (double) data->input->x / data->win->w * 2 - 1;
	c.y = (double) data->input->y / data->win->h * 2 - 1;
...
void		draw_julia(t_data *data)
{
	...
	while (++c.y < data->win->h)
	{
		...
		{
			n.x = ((c.x / data->win->w - 0.5) * data->zoom);
			n.y = ((c.y / data->win->h - 0.5) * data->zoom);
			max_i = 50 - log(data->zoom);
			draw_pixel(data, n, c, max_i);
		}
	}
}
